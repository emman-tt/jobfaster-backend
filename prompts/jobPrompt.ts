...
export const applyJobPromptUpload = (data: ApplyJobData) => {
  return `
...
## EMAIL DETAILS
- Hiring Manager: ${data.hiringManager && data.hiringManager.length > 0 ? data.hiringManager : "Unknown"}
...
3. **Greeting** — ${
    data.hiringManager && data.hiringManager.length > 0
      ? `Use "Dear ${data.hiringManager},"`
      : 'Extract company name from job description and use "Hi [Company Name] Team,"'
  }
...
export const applyJobPromptEditor = (data: any) => {
